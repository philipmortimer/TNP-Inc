...
def update_kv_cache(k_new, v_new, cache: Optional[dict], id):
    if cache is None: return k_new, v_new # Training - in case of an empty cache, k and v are simply returned

    k_curr, v_curr = cache.get(id, (None, None)) # Gets previously cached k and v values (or (None, None) if layer has not been cached yet)
    ...
    cache[id] = (k_updated, v_updated) # Updates cache
