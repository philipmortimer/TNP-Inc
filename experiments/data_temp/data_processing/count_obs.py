...
def cache_n_rows(split_dir, min_needed, max_cap, show: bool=False):
    ...
    # Builds cache
    tmp_dir = split_dir / f".tmp_minn_{min_needed}"
    tmp_dir.mkdir(parents=True, exist_ok=True)
    counts_file_tmp = tmp_dir / "counts.parquet"
    ...
    con.execute(f"""
        ...
        TO '{counts_file_tmp.as_posix()}'
        (FORMAT PARQUET, COMPRESSION 'snappy')
    """)
    # Get number of readings at each time point
    n_rows, max_practical = con.execute(
        f"SELECT COUNT(*), max(n_obs) FROM read_parquet('{counts_file_tmp.as_posix()}')"
    ).fetchone()

    if n_rows == 0:
        counts_file_tmp.unlink(missing_ok=True)
        sys.exit(f"no times satisfying min_needed={min_needed} found.")

    # Rename the temp file to the new max now we now the strict upper bound
    cache_dir = split_dir / f"minn_{min_needed}_maxn_{max_practical}"
